...
def chunk_document(document_id: str, strategy: str | None = None) -> dict:
    with SessionLocal() as session:
        ...
        try:
            ...
            # Covers both chunking-computation failures (bad strategy config,
            # validation) and persistence failures (e.g. a parser's
            # heading-detection heuristic producing a value too long for a
            # column) — either way the document must not be left stuck in
            # CHUNKING forever with no feedback, which is what happened
            # before this except clause covered the persistence phase too.
            session.rollback()
